...
public class StaticMongoEntity implements MongoEntity {
    ...
    private DBObject _format_object(Object o) {
        ...
        for (String key : map.keySet()) {
            ...
            if (ss.length > 1)
                try {
                    arrayOffset = Integer.parseInt(ss[1]);
                }
            ...
            // 未知的键，抛错
            // if (null == mef)
            // throw Lang.makeThrow("Unknow key '%s' in collection '%s'", key,
            // collectionName);
            ...
            if (null == mef) {
                dbKey = key;
                dboMap.put(dbKey, val);
            } else {
                dbKey = mef.getDbName() + (arrayOffset >= 0 ? "." + arrayOffset : "");
